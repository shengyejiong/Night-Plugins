...
public Action MenuFunc_Snd(int client, int args)
{
	...
	AddMenuItem(menu, "item1", "关闭命中反馈（无音效、无图标）");
			...
			case 1:
				{
					...
					PrintToChat(client,"已关闭命中反馈（无音效、无图标）");
				}
...
{
	// SoundSelect == 1 表示该玩家关闭了命中反馈，声音和图标都不显示。
	if (!IsValidClient(client) || SoundSelect[client] == 1)
	{
		return;
	}
