...
int    g_iReportTime, g_iDisplayNumber, /*g_iPlayerNum, */g_iChapterTotal[2], g_iCumulativeTotal[2], g_iKillSpecial[MAXPLAYERS+1], g_iKillZombie[MAXPLAYERS+1], g_iDmgHealth[2][MAXPLAYERS+1], g_iPlayerHP[MAXPLAYERS+1];
...
char g_sTitle[][] = {"状态", "难度", "血量", "特感", "弹药", "丧尸", "被黑", "友伤", "名字"};//这里最好不要改变长度,否则自动对齐可能怪怪的.
...
public Plugin myinfo = 
{
	...
	author 			= "豆瓣酱な | HUD的include提供者:sorallll | らくらく安楽死 | night",
...
public void OnPluginStart()
{
	...
	g_hTypeRanking		= CreateConVar("l4d2_emshud_ranking_Type", "511", "排行榜显示那些内容(需要启用的功能数字相加). 0=禁用, 1=状态, 2=难度, 4=血量, 8=特感, 16=弹药, 32=丧尸, 64=被黑, 128=友伤, 256=名字, 511=全部.", CVAR_FLAGS);
	g_hDispRanking		= CreateConVar("l4d2_emshud_ranking_disp", "63", "那些内容的值都为<0>时自动隐藏(需要自动隐藏的功能数字相加). 0=显示, 1=血量, 2=特感, 4=弹药, 8=丧尸, 16=被黑, 32=友伤, 63=全部.", CVAR_FLAGS);
	g_hRulesRanking		= CreateConVar("l4d2_emshud_ranking_rules", "2", "设置用什么类型排序. 1=血量, 2=特感, 3=弹药, 4=丧尸, 5=被黑, 6=友伤.", CVAR_FLAGS);
...
void IsKillLeaderboards()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2)
		{
			...
			int target = !iBot ? i : iBot;
			
			assisters[assister_count][0] = target;
			assisters[assister_count][1] = GetSurvivorHP(i);
			assisters[assister_count][2] = g_iKillSpecial[target] > 9999 ? 9999 : g_iKillSpecial[target];
			int totalAmmo = GetPlayerTotalAmmo(target);
			assisters[assister_count][3] = totalAmmo > 9999 ? 9999 : totalAmmo;
			assisters[assister_count][4] = g_iKillZombie[target] > 9999 ? 9999 : g_iKillZombie[target];
			assisters[assister_count][5] = g_iDmgHealth[0][target] > 9999 ? 9999 : g_iDmgHealth[0][target];
			assisters[assister_count][6] = g_iDmgHealth[1][target] > 9999 ? 9999 : g_iDmgHealth[1][target];
			assister_count++;
		}
	}
	//以设定的变量排序.
	...
	for (int x = 0; x < assister_count+1; x++)
	{
		...
		if (IsValidClient(client))//因为要显示闲置玩家的数据,所以这里不要判断团队.
		{
			...
			IntToString(iAmmo,		sData[4][j], maxStringLength);
...
// 获取玩家主武器总弹药（当前弹匣 + 备用弹药）
stock int GetPlayerTotalAmmo(int client)
{
	if (!IsPlayerAlive(client))
		return 0;
	...
	if (weapon > MaxClients && IsValidEntity(weapon))
	{
		...
		if (iAmmoType != -1)
			reserve = GetEntProp(client, Prop_Send, "m_iAmmo", _, iAmmoType);
